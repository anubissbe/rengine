	...
	return {
		...
		async fetchContexts(projectId: string) {
			...
			try {
				...
				error = errorMessage(e, 'Scan contexts not loaded');
			} finally {
				isLoading = false;
			}
		},
		...
		async createContext(
			...
			try {
				...
				error = errorMessage(e, 'Scan context not created');
				return null;
			}
		},
		...
		async updateContext(
			...
			try {
				...
				error = errorMessage(e, 'Scan context not saved');
				return null;
			}
		},
		...
		async deleteContext(id: string, projectId?: string): Promise<boolean> {
			...
			try {
				...
				error = errorMessage(e, 'Scan context not deleted');
				return false;
			}
		},
		...
		async duplicateContext(id: string, projectId: string): Promise<ScanContextRead | null> {
			...
			try {
				...
				error = errorMessage(e, 'Scan context not duplicated');
				return null;
			}
		},
