...
>
	<div class="flex min-w-0 flex-col gap-1">
		<span class="flex flex-wrap items-center gap-2 text-xs font-medium">
			<span
				class="inline-flex items-center gap-1 rounded border border-info/40 bg-info/10 px-1.5 py-px text-2xs text-info"
			>
				<Sparkle class="size-2.5" />
				Suggested rule
			</span>
			{suggestion.name}
			<span class="text-2xs font-normal text-muted-foreground">{suggestion.kind_label}</span>
		</span>
